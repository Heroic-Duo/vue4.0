    ...
    <div class="zhaohui">
      <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80">
        <FormItem prop="yzm" label="验证码">
          <div class="yzmImg">
            <img @click="yym()" :src="imgSrc" class="form-group-yz-img" />
          </div>
        ...
        <FormItem prop="password" label="修改密码">
          <Input v-model="formValidate.password" placeholder="修改密码"></Input>
        </FormItem>
...
export default {
  ...
  data() {
    return {
      //图片验证码
      imgSrc: "/api/apiUser/forget/code",
      formValidate: {
        mail: "",
        password: "",
        yzm: "",
  ...
  methods: {
    yym() {
      ...
      this.imgSrc = "/api/apiUser/forget/code?t=" + sj;
    },
    handleSubmit() {
      this.$refs.formValidate.validate(valid => {
        if (valid) {
          this.$axios
            .post("/api/apiUser/forget/index", {
              headers: {
